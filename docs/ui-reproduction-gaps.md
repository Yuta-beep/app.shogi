...
- [x] 駒詳細オーバーレイ（駒名・スキル説明・行動範囲テキスト）
- [x] デッキ保存UIモーダル（名前入力 → 保存）
- [x] デッキロードUIモーダル（保存済み一覧・削除）
- [x] デフォルトデッキ読込確認モーダル
- [ ] アンドゥ機能（盤面編集未実装のためスキップ）

---

## PIECE INFO (`piece_info.html` → `src/features/piece-info/`)

- [x] 移動範囲グリッド可視化（5x5グリッド、moveVectors から生成）
- [~] 下部ナビゲーション（前後ボタン＋カウンターあり、サムネイル行なし）
- [x] 解放条件テキスト表示
